...
export function useWeather(cityName) {
  ...
  const fetchForecast = useCallback(async (city) => {
    try {
      const res  = await fetch(`${BASE_URL}/forecast?q=${encodeURIComponent(city)}&appid=${API_KEY}&units=metric&cnt=40`);
      if (!res.ok) return;
      const data = await res.json();

      // One reading per day, closest to midday
      const days = {};
      ...
      setForecast(
        Object.entries(days).slice(0, 5).map(([day, item]) => ({
          day,
